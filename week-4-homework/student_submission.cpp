...
void simulate_waves(ProblemData &problemData) {
	...
	for (int x = 1; x < MAP_SIZE - 1; ++x) {
		for (int y = 1; y < MAP_SIZE - 1; ++y) {
			...
			float acceleration = ATTACK_FACTOR * (lastWaveIntensity[x][y - 1]
				+ lastWaveIntensity[x - 1][y]
				+ lastWaveIntensity[x + 1][y]
				+ lastWaveIntensity[x][y + 1])
				- (4 * ATTACK_FACTOR - 1) * lastWaveIntensity[x][y]
				- secondLastWaveIntensity[x][y];
...
bool findPathWithExhaustiveSearch(ProblemData &problemData, int timestep,
 ...
 // omp_set_dynamic(0);
    ...
    //static is to avoid the NUMA problem, dynamic will cause sockets switching
    #pragma omp parallel for schedule(static, 32) num_threads(16) proc_bind
	for (int x = 0; x < MAP_SIZE; ++x) {
		for (int y = 0; y < MAP_SIZE; ++y) {
			...
			for (Position2D &neighbor : neighbours) {
                ...
                //could use OR here to search all the neighbors??? instead of add 
